...
export const initialOverviewState = {
  ...
  coldChainOverviewData: { isLoading: true },
  months: [],
};
...
export const OverviewContextProvider = ({ children }) => {
  ...
  const getOverviewColdChainData = async (year) => {
    dispatch({
      type: 'LOADING_OVERVIEW_COLD_CHAIN_DATA',
      payload: { isLoading: true },
    });

    try {
      const coldChainOverviewData = await axios.get(
        `http://${apiEndpoint}${port}/coldchain/api/overviewstats?year=${year}`,
      );

      const payload = {
        coldChainOverviewData: coldChainOverviewData.data,
        isLoading: false,
      };

      dispatch({
        type: 'SET_OVERVIEW_COLD_CHAIN_DATA',
        payload: payload,
      });
    } catch (err) {
      dispatch({
        type: 'GET_DATA_ERROR',
        payload:
          'An error occured getting cold chain overview  data from backend',
      });
    }
  };
  ...
  return (
    <OverviewContext.Provider
      value={{
        ...
        coldChainOverviewData: state.coldChainOverviewData,
